...
Browse and sell on Facebook Marketplace: search listings, list your own listings,
view item details, get seller info, and create, edit, delete, and publish your
listings.
...
```sh
facebook-cli marketplace search --query "<query>" --max-price <price> --limit <n>
```
...
1. **Listing image** — render inline using `![Title](image_url)` markdown. If `image_url` is present, always include it. If absent, omit the image — never invent image URLs.
2. **Title** and **Price**
3. **Description snippet** (brief excerpt from the listing description)
4. **Location**
5. **Listing link** — use the `product_url` field directly (fall back to `https://www.facebook.com/marketplace/item/<listing_id>/` if absent)
...
Format as a numbered list. Include the image at the top of each listing entry so the user can visually browse items.

### Step 4: Item details (on request)

```sh
facebook-cli marketplace listing details --listing-id <LISTING_ID>
```

Present: listing image (`![Title](photo_url)` if available), full description, price and condition, seller info, creation date, and link to listing (`url` field).
...
```sh
facebook-cli marketplace saved [--keywords <text>] [--limit <n>] [--after <cursor>]
```
...
page. Max page size is 20.

### Step 7: My listings (on request)

```sh
facebook-cli marketplace my-listings [--status active|pending|sold|draft] [--limit <n>] [--after <cursor>]
```
...
- `image_url` (string, nullable) — Primary photo URL (may be null)
...
- `seller_photo` (string) — Seller profile photo URL
...
- `profile_picture` (string) — Profile photo URL (may be empty)
...
- **location** — geographic coordinates, passed as `--latitude` and `--longitude` **together** *(publish gate — required to publish)*. There is **no** `--location`, `--city`, or `--address` flag — geocode any place name (e.g. "San Jose, CA") to a lat/lng pair first, then pass the two numeric flags.
...
7. Always render listing images inline using `![Title](image_url)` when `image_url` is present. Never invent image URLs. (Item Detail responses use `photo_url`.)
8. Always include a clickable listing link. For search results and my-listings, use the `product_url` field (fall back to `https://www.facebook.com/marketplace/item/<listing_id>/`). For item details, use the `url` field directly.
