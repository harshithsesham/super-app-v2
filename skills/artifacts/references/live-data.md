...
build request's `Refresh triggers`. Register the job from the builder seat with
`owner: space:<slug>`, record it in `space.json` `managedCronJobs`, and have the
job body invoke the artifact's declared refresh action. Scheduled runs execute
unattended under the same web-reading approval.
...
- Source the data during the build with the bundled CLI: run
  ...
  rides. Add `--since` for recency and `--verticals finance|sports|weather`
  for typed domains, one ticker per finance query.
