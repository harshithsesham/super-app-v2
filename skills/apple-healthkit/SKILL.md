...
title: "Apple Health"
description: "The user's synced Apple Health (HealthKit) data from their iPhone or Apple Watch: daily metrics (steps, distance, calories, heart rate, HRV, VO2max), sleep sessions (stages, quality, efficiency), and workouts."
...
iPhone or Apple Watch, it has no data for them — check the `platform` field
...
```
HealthKit has no account to connect/disconnect — data is device-synced. Both
return device-sync guidance:
`{ ok, auth_type:"device_sync", requires_connect:false, instruction }`.
