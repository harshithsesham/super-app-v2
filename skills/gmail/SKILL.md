...
exact dotted API method). Use the same `--account <account_id>` selection as the
...
- List candidates with their sender, subject, and date: `+triage --query '<query>' --max 50 --format json`.
...
- Send with the content in the command (`+send`, `+reply`, `+forward`), not by saving a draft and sending it by id (`users drafts send`). The approval is built from the command, so a content-bearing send lets the user see the real recipient and body, while a draft id shows them nothing to check.
