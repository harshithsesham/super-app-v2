...
6. **Confirm explicit user intent before sending.** For a `send`, require the exact message content and the destination thread or recipients from the user; for a reply, require a concrete source message and explicit body text. Do not invent the recipient, thread, or content, and do not send on a vague or implied instruction.
