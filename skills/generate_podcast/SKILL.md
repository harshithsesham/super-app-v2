...
**Language:** podcasts default to English. For another language, **write the script in that language** and pass `--language` (e.g. `--language es`, `--language pt`, or a locale like `pt_BR`). Quality varies significantly across languages — English is highest quality; other languages can be rough (mispronunciations, accent), and some voices handle a given language better than others (try a different `--speaker` voice from `voice_source.json` if one sounds wrong). Tell the user non-English audio may be imperfect. See the `tts` skill's Language section for details.
...
Publishing unifies all episode artwork to the feed's cover image — individual episode and series covers are replaced with the feed cover in the podcast catalog.
...
**Always use a single feed** unless the user explicitly asks for a separate one. Before publishing, run `podcast-helper manifest read` and check the `feed` object's `feed_url`; if it is non-empty, reuse that feed's `feed_title` exactly.
...
| Published episode | All episode covers unified to the feed's cover image |
