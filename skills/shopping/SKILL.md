---
name: "shopping"
description: "Use for any product or shopping question: find, reverse image search, buy, compare, or evaluate real products with prices, images, and product page URLs, including buying or browsing Facebook Marketplace listings. Use when presenting shopping search results from any source."
metadata:
...
Resolve each one in this order: what the user said in this request or earlier in the conversation, then `~/USER.md` (already in your context), then `muse.memory_search` for durable preferences and sizes. `~/USER.md` and remembered sizes describe the user, so they settle an attribute only when the user is the wearer; when the item is for someone else, use what the user says about that person and their `~/memory/people/` page. Never infer the wearer's gender from the user's name, and never fall back to a default.
...
1. Identify which product to search for from the image; ask a clarifying question if necessary.
2. Derive a detailed visual description.
3. Execute searches with the relevant product search tools with queries that leverage the detailed visual description.
4. Verify product pages / images against the original image; rank exact matches ahead of visually similar results.
5. Continue searching and adjusting your queries as necessary in order to identify the product. Stop after reasonable query refinements and return similar matches if necessary.
6. Present the relevant search results to the user. Be honest with the user if you're unable to find an exact match.
...
3. For Meta catalog products with `is_agentic_checkout_creation_enabled: true`, load `/opt/hatch/skills/shopping/references/shopify-ucp.md` and follow its purchase flow. It creates the checkout first, then asks the user to choose between Shop Pay and Stripe Link, and needs a connected Link only if they pick Link.
4. Otherwise, load `/opt/hatch/skills/shopping/references/browser-checkout.md` and follow Purchasing Flow. Resolve item choices together while browsing, before starting checkout. Include selected shipping in the final review.
