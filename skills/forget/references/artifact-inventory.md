...
Include message attachments, reactions, native-channel copies, shared-agent or
delegate-room messages, approval requests or decisions, and any pending handoff
that can reinsert the content.
...
staging artifact can reapply it. Quiesce an in-flight writer first. A retained
run receipt may keep content-free provenance, but any model-readable evidence
...
cache, health or sensor records, call logs, ambient audio or image summaries,
...
3. standing prompt context, new compaction summaries, memory flushes, and
   restart checkpoints do not preserve or restate it;
4. no active or scheduled producer can recreate it;
