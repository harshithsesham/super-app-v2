...
name: "muse-feedback"
title: "Muse feedback"
description: "Whenever a response of yours tells the user you can't do a specific thing they wanted, or accepts that they're giving up on one, that same response should offer to file it as a feature request with the Muse team. File only on the user's go-ahead, and offer once. This covers missing integrations that you couldn’t do yourself, capability you lack or a task that keeps failing at a specific place. Also use this skill to view, check, or withdraw previously filed reports."
...
note; nothing in it is published anywhere. A feature request is worth filing
only when it names something the Muse team can't otherwise see: which service,
site, device, or capability this user actually needed.
...
for a site the name without the `.com`. `missing-capability` is for gaps
with no outside service involved.

Never file:

- Muse itself being slow, expensive, limited, or broken right now: signing
  in to Muse failing, outages, model quality. Muse already measures these
  itself. (Connecting to an outside service failing is different: that is
  the give-up gap above.) Slowness stays unfiled even when it names a site;
  `broken-behavior` is only for tasks that actually fail.
- An action Muse deliberately refuses, like moving the user's money. The
  boundary is intentional; never offer to file against it.
- Errors that recovered. If a retry worked, or the conversation hasn't named
  where it keeps failing, nothing is filed.
- Your own execution mistakes. Filing a report is not an apology; fix or
  retry instead.
...
When you decline to file, say it in the user's own terms, e.g. "Muse
already measures its own speed automatically, so a note from me wouldn't
tell the Muse team anything new." Never say "telemetry", "fleet", or
"ledger" to the user.
...
app overall. Follow the same filing and repeat-report rules below.
...
1. Describe the feedback in terms any user could share, free of private data:
   ...
   Use details already in the conversation. If it does not identify a
   specific subject, do not file. Ask only for approval to send the report;
   do not ask the user for additional report details.
...
2. Quote the returned summary in full, verbatim, including the service name.
   Ask whether to send it to the Muse team as a private note, and say that the
   report excludes details about the user's own situation. End your turn. Do
   not file until the user explicitly approves in a later turn. Ask once. If
   they decline or do not answer the offer, drop it for the rest of the
   conversation, including the goodbye.
