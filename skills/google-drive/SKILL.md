...
- New folder: `drive files create --json '{"name":"Q3 Docs","mimeType":"application/vnd.google-apps.folder","parents":["<parent_folder_id>"]}'`. Use `"root"` as the parent for the top level.
- Rename: `drive files update --params '{"fileId":"<id>"}' --json '{"name":"Q3 Budget"}'`.
- Move: `drive files update --params '{"fileId":"<id>","addParents":"<dest_folder_id>","removeParents":"<current_folder_id>"}'`. Read the current parent from a `files get` first so you remove the right one.
- Copy: `drive files copy --params '{"fileId":"<id>"}' --json '{"name":"Copy of Q3 Budget"}'`.
...
- Creating or uploading in My Drive or a private folder, changing a private item, trashing, restoring, and deleting may proceed from a clear, unambiguous user request without an additional confirmation. Confirm before creating or uploading in a shared folder, sharing, changing permissions, or modifying an item that is already shared because those actions can expose content to or change it for other people.
