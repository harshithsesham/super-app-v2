...
2. Build the candidate set from the live roster. An explicitly named device
   must resolve unambiguously. Otherwise inspect the device in the current
   `[device_id=<node_id>]` tag first; with no tag, use the sole paired device or
...
3. Call `device.describe` before deciding whether a candidate supports the
   ...
   commands and need no special prefix. For an action, ask which device to use
   when several candidates advertise a matching command. If the tagged device
   lacks it, ask before switching to another device unless the user already
   chose that device.
...
5. Call `device.invoke` for the selected device and command. Report success
   ...
   permission, availability, or execution errors in user-facing language.
