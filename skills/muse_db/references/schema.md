...
Migration-set fingerprint: `44fa974198650c59b022c266d7d9e7cc53ff4cd8a677859c1fc1aa39b4ea7e28`.
...
| `feed_id` | `ideas.idea_card_feeds.feed_id` |
...
| `singleton` | `boolean` | no | `true` | Local row identifier (primary key). |
| `feed_title` | `text` | no |  |  |
| `feed_id` | `text` | yes |  | Remote podcast-storage feed identifier; no Muse PostgreSQL owner table. |
| `feed_url` | `text` | yes |  |  |
| `cover_path` | `text` | yes |  |  |
| `updated_at` | `timestamp with time zone` | no | `now()` |  |
...
| `owner_agent_id` | `text` | no |  | Soft local reference → `agent.agents.agent_id`. |
| `root_message_id` | `text` | no |  | Soft local reference → `runtime.messages.message_id`. |
| `stream_owner_message_id` | `text` | no |  | Soft local reference → `runtime.messages.message_id`. |
| `status` | `text` | no |  |  |
