---
name: "instagram"
description: "Read the user's Instagram account: profile, followers and following, posts and their comments and likes, stories, feed, saved content, and insights. Can tune feed ranking, update the profile bio and picture, and publish stories, reels, feed posts, and carousels on request."
icon: "instagram"
...
## Operating rules
1. This skill is for the authenticated user's own Instagram non-messaging data and location lookup for Story stickers, not general people or post search.
...
### Followers
Omit `--user-id` to fetch the user's own follower list. `--count` is optional and defaults to `200`.
...
instagram-cli followers --account-id <user_own_fbid> --user-id 182294127 --count 25
instagram-cli followers --account-id <user_own_fbid> --user-id 182294127 --count 25 --after <cursor>
```
...
### Post by ID
Fetch a single post by its media ID.
...
### Account insights
Fetch account-level insights. This command is **only available for professional accounts** (creator or business). The `accounts` command will tell you whether the user's account is professional — check before using this command.

`--user-id` is the user's FBID (fetch via `accounts` if unknown). `--start-time` and `--end-time` are Unix timestamps **in seconds**.

```sh
instagram-cli account-insights --account-id <user_own_fbid> --user-id 313201324 --start-time 1709269200 --end-time 1711947600
```
