...
## Voice Catalog
The voice catalog is `/opt/hatch/skills/voice-selector/voice_source.json` — the same catalog used for voice calls and the voice picker. Each entry has an `id` (e.g. `avocado_v2:briggs`), a display `name` (e.g. "Briggs"), and a description. Read that file to choose a voice, then pass its `id` to `--voice`/`--speaker` exactly as the catalog writes it (colon form) — never retyped, abbreviated, or reshaped. The one exception is a complete voice id the user supplies and knows works; pass that through unchanged too.
...
When the content, persona, or user request steers toward a different catalog
voice — a thematic fit, an accent, a gender, a particular vibe — it's
perfectly fine to pick that voice from `voice_source.json` instead. Always
honor an explicit voice request.
...
- `--voice <VOICE_ID>` — primary voice ID (a catalog `id` as written), default `avocado_v2:MAI_03` (Smooth)
...
- **Do not switch to a different voice**, and **do not switch to a different TTS
  engine or endpoint.** A voice taken from `voice_source.json` was valid; a failure is
  not a reason to pick another voice or reach for a different synthesizer.
...
- `HTTP 500 Internal Server Error: ---THERE WAS AN ERROR---` is the backend's opaque
  ...
  nothing in the response separates the two causes. **Check the voice ids first**, by
  this test: open `voice_source.json` (see [Voice Catalog](#voice-catalog)) and confirm
  that every id in the failed command — `--voice`, `--voice2`, and the `voice_id` half
  of each `--speaker Name=voice_id` — appears in it character-for-character. An id you
  retyped, abbreviated to its suffix, reshaped, or recalled from memory will not match,
  and by far the most common cause of this error is exactly that. A mismatched id never
  clears on its own: correct it to the catalog spelling and resend. Only when every id
  matches the catalog exactly is the backend itself at fault — then run the ladder above
  with the request completely unchanged.

## Operating Rules
1. Always write audio to a workspace path or another explicit local path.
2. Choose voices from `/opt/hatch/skills/voice-selector/voice_source.json` by their catalog `id`. Copy the `id` through verbatim; the backend accepts it as-is. Default to the Meta AI voices (`avocado_v2:MAI_01` and `avocado_v2:MAI_03`) most of the time, but pick another catalog voice when the content, persona, or user request steers that way. Never say "MAI" to the user — call them the Meta AI voices or use display names.
