...
- [x] Secure Store (Muse's Secure Vault): entry cards for site logins, new passwords, and API keys; a signed secure page writes to the cell's encrypted vault; the agent sees metadata only and the browser worker fills fields with `fill_credential` while snapshots mask password inputs and scrub filled secrets; vault keys are born on the cell volume, never held by the gateway (`superapp/connectors/credstore.py`, `superapp/tools/credentials.py`, `apps/mobile/src/ui/CredentialCard.tsx`)
