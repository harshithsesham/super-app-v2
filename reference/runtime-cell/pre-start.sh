...
# /run/hatch/sandbox-api hosts the daemon's restricted sandbox HTTP API
# Unix socket (api.sock). The dir is bind-mounted into the runtime cell so
# in-cell processes can dial a small allowlist of auth-free routes (the
# canonical list lives in `build_sandbox_api_router` in hatch-server's
# http_api.rs); everything else returns 403. Owned by hatch-daemon (the
# daemon process inside the cell must create + bind api.sock here).
#
# Mode 0755 is required because non-daemon in-cell consumers (Spaces /
# sandbox skill workers / anything mapped to a host UID outside the
# hatch-daemon group via PrivateUsers=131072) need o+x to traverse INTO
# the dir to reach the socket. The daemon itself is the owner so dir
# perms don't gate it; the o+x bit is what enables non-owner in-cell
# consumers. Access control on this surface is the route allowlist
# (server-side) — not the dir mode.
#
# !!! SECURITY LEAK in the "in-cell auth bypass removal" abstraction.
# 0755 + the default 0666 socket also lets host-local UIDs (operator
# SSH, monitoring agents, any future host-side process) connect to
# the sandbox API. This contradicts the stated goal of "in-cell
# processes may not call the broad daemon HTTP API without auth"
# because the sandbox API exposes a no-auth allowlist that grants
# the same practical access for state-mutating routes (goals CRUD,
# ideas create, etc.) — see the SECURITY warning at the top of
# `build_sandbox_api_router` in http_api.rs. One PR adding a new
# host-side service, or one regression that lifts the install-time
# UID restrictions, widens the blast radius silently. Each new entry
# in the sandbox allowlist must be audited as if it were a
# host-reachable unauthenticated endpoint.
...
# bind-mounted from the host. The postgres socket is proxied by the seccomp
