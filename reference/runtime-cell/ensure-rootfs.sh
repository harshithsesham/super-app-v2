...
if [ -f "/etc/hatch/env.override" ]; then
    ...
    # gets the same secret-shaped-key scrub the base env file gets spawnd-side
    # (runtime_env_key_forbidden in environment/credentials.rs — keep the
    # marker list below in lockstep with it). The HOST file is untouched:
    # operators may park secret-shaped emergency keys there (0600, host-only);
    # they must never ship verbatim into an attacker-readable rootfs on the
    # next cell start. Case-insensitive; a scrubbed line is replaced by a
    # comment naming the key so in-cell debugging sees the removal.
    scrubbed_override=$(mktemp)
    awk '
        /^[[:space:]]*(#|$)/ { print; next }
        {
            line = $0
            ...
            ukey = toupper(key)
            if (ukey ~ /SECRET|TOKEN|API_KEY|DATABASE_URL|PASSWORD|PRIVATE_KEY|CREDENTIAL/ ||
                ukey == "GH_TOKEN" || ukey == "GH_TOKEN_FILE") {
                print "# scrubbed for cell copy: " key
                next
            }
            print line
        }
    ' "/etc/hatch/env.override" > "$scrubbed_override"
    safe_install 0644 "$scrubbed_override" "$rel_env_override_file"
    rm -f "$scrubbed_override"
else
